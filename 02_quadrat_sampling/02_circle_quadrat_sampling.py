...
def create_random_circle_quadrat(rectangle, radius):
    """
    Generate a random circular quadrat of given radius fully within a boundary polygon.
    """
    minx, miny, maxx, maxy = rectangle.bounds
    while True:
        # Randomly select a center point within the rectangle
        random_x = random.uniform(minx + radius, maxx - radius)
        random_y = random.uniform(miny + radius, maxy - radius)
        circle = Point(random_x, random_y).buffer(radius)

        if rectangle.contains(circle):
            return circle

def calculate_circle_density(rectangle, points_gdf, radius, num_quadrat):
    """
    Computes the density of points within randomly placed circular quadrats inside a given rectangle.
    Returns an array of densities computed over multiple quadrats.
    """
    sampled_circles = [create_random_circle_quadrat(rectangle, radius) for _ in range(num_quadrat)]
    circles_gdf = gpd.GeoDataFrame(geometry=sampled_circles, crs="EPSG:2193")
    circle_area = np.pi * radius ** 2
    points_per_circle = [len(points_gdf.sindex.query(circle, predicate='contains')) for circle in circles_gdf.geometry]
    densities = np.array(points_per_circle) / circle_area
    return densities

def perform_circle_quadrat_sampling(parquet_file, radii, num_quadrats):
    """
    Loads a parquet file and performs circle quadrat sampling to estimate point density.
    ...
    for radius in radii:
        for num_quadrat in num_quadrats:
            if radius * 2 > row_width:
                print(f"Skipping file: quadrat size is larger than row width.")
                return []
            densities = calculate_circle_density(rectangle, points, radius, num_quadrat)
...
def process_selected_files(file_paths, radii, num_quadrats):
    """
    ...
    for parquet_file in file_paths:
        results = perform_circle_quadrat_sampling(parquet_file, radii, num_quadrats)
...
if __name__ == "__main__":
    ...
    df = process_selected_files(selected_files, [0.5, 1, 1.5], [10, 50, 100, 200])
    aggregated_df = df.groupby(["Circle Radius (m)", "Number of Quadrats"]).agg({
        "Bias": "mean",
        "Variance": "mean",
